...
1. **The container to be profiled must be started privileged.**
1. **Note that Framework symbols won't resolve by default. There are two options:**
...
1. Start the container privileged and with env vars set:
...
2. Start docker container with /tmp mapped to a shared volume.
    1. `docker run -d -p 8080:80 -v /tmp/container_sockets:/tmp --rm testapp`
3. Start (and exec into) the sidecar container with the same process space and shared /tmp.
...
1. Exec into sidecar container
    1. `kubectl exec -it testapp -c sidecar -- /bin/bash`
1. In AKS, it may be necessary to edit /usr/bin/perf to have a correct version (4.19). It used to be necessary to build perf manually, but that seems to not be the case anymore.
1. Collect
    1. `./perfcollect collect sampleTrace`
1. Exit container and copy file locally
